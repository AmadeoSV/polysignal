...
### Round One: Detection Was Never Actually Running
...
**Where this left Kalshi:** detection and resolution trustworthy, and every signal above a small noise floor (0.5¢ move, $100 depth — see the main README's [Signal Logic](#signal-logic) for how this differs from the alert threshold) being saved for research — the same "collect broadly, find the edge empirically" approach that worked for Polymarket, rather than guessing at per-market thresholds up front.
...
**The last bug, found on a final audit pass, not a live symptom.** `fetch_markets()` requested only the first 20 open markets per series, per cycle, with no pagination — confirmed against Kalshi's own docs that `/markets` is cursor-paginated and a single call only returns page one. On any day with a full slate (a normal MLB day is ~15 games × 2 tickers), markets past position 20 were silently invisible to the scanner from the moment they opened, not just slow to resolve — confirmed concretely: a real, finalized MLB game had zero rows in the signals table, ever. Fixed with real cursor-following pagination, plus a per-series sub-cap so one busy sports day can't now crowd out every other category the same way sports itself was originally crowded out.

**Where this left things:** Kalshi's deduplicated resolved sample at 35W/35L (578 pending, drawn from a research-capture pool of ~8,400 signals) — a real, live, steadily-growing number now that the pipeline was fixed end to end, not yet a large enough sample to say anything about an edge. The pattern across both audit passes: assume nothing is silently correct just because it isn't throwing an error, and check every fix against live data before trusting it.

### Round Three: Resolved Doesn't Mean Finished

Fixing detection didn't fully fix resolution. A live MLB game, still showing "LIVE, 7th inning" on Kalshi's own site, turned out to already have a WON/LOST outcome recorded in the database — written nearly two hours earlier, and rewritten several times since.

**The cause:** `check_signal_outcomes()` treated Kalshi's `result` field as authoritative the moment it was non-empty, without checking `status`. Kalshi's real lifecycle is `active → closed → determined → finalized` (`disputed`/`amended` are also real states), and `result` can populate before a market reaches `finalized` — and can still change after that. Tracing one ticker's full history confirmed it directly: 27 signal rows for the same game, `WON`/`LOST`/blank written and overwritten across two hours, all while the game was still being played. The underlying comparison logic (`bullish == resolved_yes`) was correct the entire time — every value written was internally consistent with whatever `result` said at that moment. The bug was trusting that field before it was actually locked in.

This is the same class of mistake as the original Polymarket bug — a market treated as closed before it genuinely was — just surfacing through a different mechanism. The fix there was checking a `closed` boolean before trusting a price. The fix here is checking `status == "finalized"` before trusting `result`; anything else is treated as still pending, same as an empty result.

**Auditing the existing data, not just fixing it going forward:** since this bug predates every other change made during this pass, every already-resolved Kalshi outcome was suspect, not just the one game that surfaced it. Built a read-only audit tool (`audit_kalshi_outcomes.py`, same pattern as the original Polymarket audit script) that re-checks every distinct resolved `(ticker, direction)` pair against Kalshi's live API and classifies each as correct, wrong, or resolved-before-finalized. Result: **192/192 checked clean** — zero wrong outcomes, zero still-unfinalized, across all four categories. The bug fired (directly proven), but nothing in the current data is left disagreeing with ground truth.

One honest limit on that result, worth stating plainly: the audit can only compare stored values against Kalshi's *current* state. A value that was wrong when written but has since been silently overwritten by a later, correct check — or one that happened to guess right — leaves no trace to catch after the fact. That gap is closed going forward: `signals` now has a `resolved_at` timestamp, added specifically because tracing this bug required knowing *when* an outcome was written and that information didn't exist. Every future resolution records it.
